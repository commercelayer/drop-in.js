...
This component is the owner of the price information. It fetches the price of the specified [SKU](https://docs.commercelayer.io/core/v/api-reference/skus) or [bundle](https://docs.commercelayer.io/core/v/api-reference/bundles) and dispatches it to the [`cl-price-amount`](?path=/docs/components-price-cl-price-amount--docs) children.
...
<Alert title="Bundles" type="info">
  The pre-filled code in the *Control* field refers to an SKU. If you select <code>bundle</code> as the kind of item, make sure to change it with a bundle code (e.g. <code>{codes.bundleAvailable}</code>) so that the example above shows the correct price.
</Alert>
