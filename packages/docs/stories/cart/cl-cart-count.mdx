...
Once you have correctly set up your organization in Commerce Layer and imported the drop-in library as described [here](?path=/docs/getting-started--page), you can use the `cl-cart-count` component to display the number of items that are in your [shopping cart](?path=/docs/components-cart-cl-cart--docs), with the option to hide it in case the cart is empty. 
