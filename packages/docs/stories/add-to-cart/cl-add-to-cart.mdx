...
You can use the `frequency` attribute to select the frequency which generates a subscription. The value must be supported by existing associated subscription_model.
